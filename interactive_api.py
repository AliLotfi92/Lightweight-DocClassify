...
st.title('Document Classification with Stacking Models')
...
if st.button('Classify'):
    if message:
        ...
        st.write(f"Class: {label_encoder.inverse_transform([final_prediction])[0]}")
        ...
        for i, (pred, probs) in enumerate(zip(individual_predictions, individual_probabilities)):
            ...
            st.write(f"  Prediction: {label_encoder.inverse_transform([pred])[0]}")
            st.write("  Probabilities:")
